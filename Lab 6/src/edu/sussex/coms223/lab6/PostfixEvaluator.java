...
public class PostfixEvaluator {
	...
	public float runSteps(PrintWriter pw) throws IllegalExpressionException {
		while (exprTok.hasMoreTokens()) {
			runSome(exprTok.countTokens(), pw);// done 10: within the while loop, call runSome with the parameters 1 and pw
			     // which forces a single step to be executed.

			printStatus(pw); // done 11: within the while loop, after the call the runSome, add a call to
			// printStatus passing pw.

			pw.flush();// done 12: within the while loop, after the call the printStatus, call the
			// flush method on pw to force output to the console.

		}

		if (postStack.size() > 1)// done 13: add an if statement to check if size of postStack is greater than
		throw new IllegalExpressionException("insufficient operators"); // 1, if so throw IllegalExpressionException with the message "insufficient
		// operators".


		// done 14: replace the "return 0;" statement with a return statement that
